...
## Contents
This repository contains code for running LLM classifiers on WildChat data and making all WildChat plots from the paper. Bing Copilot data analyses are excluded as the dataset is private.
...
1. Download WildChat-4.8M from HuggingFace: https://huggingface.co/datasets/allenai/WildChat-4.8M. 
...
3. Run `python classify.py`. Expect this to take some time, so running in `screen`/`tmux`/etc is a good idea. 
Results will be saved in `wildchat-results/wildchat-4.8m-results/`. The cost of running the classification pipeline with GPT-4o-mini on WildChat-4.8M (at $0.15 / 1M input tokens, $0.60 / 1M output tokens) is ~ $3500. 
4. Run `python compute_sentence_lengths.py`.

Then run `python wildchat_plots.py` to generate the plots and tables under
`outputs/`.
