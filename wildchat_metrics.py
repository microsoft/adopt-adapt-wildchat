...
def _result_paths(data_dir, prefix, n_shards, result_type):
    ...
    for i in range(n_shards):
        path = f"{data_dir}/{prefix}-{i:05d}-{result_type}.parquet"
...
def load_metrics(dataset="wildchat-4.8m", data_dir=None, cache_path=None, use_cache=True):
    ...
    # Compute template metrics inline (reads Text column)
...
def ensure_template_metrics(m: WildChatMetrics, data_dir=None, cache_path=None,
    ...
    Reads the Text column from domain parquets, fingerprints each conversation by
    ...
    Dataset paths (data_dir, file_prefix, n_shards, cutoff) come from `m.dataset`;
    `data_dir` and `cache_path` may be passed to override.
    """
    ...
    print("Computing template metrics (reads Text column — may take a minute)...")
    _key_cols = ['conversation_hash', 'model', 'timestamp', 'turn', 'language', 'hashed_ip', 'state', 'country']
    text_df = (
        pl.scan_parquet([f"{data_dir}/{cfg.file_prefix}-{i:05d}-domain.parquet" for i in range(cfg.n_shards)])
        .select(_key_cols + ['Text'])
        .collect()
    )
    text_df = text_df.unique(subset=_key_cols)
